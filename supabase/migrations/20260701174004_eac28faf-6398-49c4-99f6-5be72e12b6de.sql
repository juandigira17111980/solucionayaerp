...
BEGIN
  ...
  -- Idempotencia: usamos un SKU marcador
  ...
  -- Geografía (usa Colombia si existe)
  ...
  -- Categorías y marcas
  INSERT INTO public.product_categories(company_id, name, code) VALUES
    (_company_id, 'Bebidas', 'BEB'), (_company_id, 'Snacks', 'SNK'), (_company_id, 'Aseo', 'ASE')
    RETURNING id INTO cat_bebidas;
  SELECT id INTO cat_bebidas FROM public.product_categories WHERE company_id=_company_id AND name='Bebidas';
  SELECT id INTO cat_snacks  FROM public.product_categories WHERE company_id=_company_id AND name='Snacks';
  SELECT id INTO cat_aseo    FROM public.product_categories WHERE company_id=_company_id AND name='Aseo';

  INSERT INTO public.brands(company_id, name) VALUES (_company_id,'Marca A'),(_company_id,'Marca B')
    ON CONFLICT DO NOTHING;
  SELECT id INTO brand_a FROM public.brands WHERE company_id=_company_id AND name='Marca A';
  SELECT id INTO brand_b FROM public.brands WHERE company_id=_company_id AND name='Marca B';
  ...
  INSERT INTO public.third_parties(company_id, document_type, document_number, legal_name, trade_name,
    is_supplier, email, phone, payment_terms_days, city_id)
  ...
  -- Terceros: clientes
  INSERT INTO public.third_parties(company_id, document_type, document_number, legal_name, trade_name,
    is_client, email, phone, credit_limit, payment_terms_days, city_id)
  ...
  INSERT INTO public.purchase_order_lines(purchase_order_id, product_id, quantity, unit_cost, tax_percent) VALUES
    (po_id, p1, 100, 3500, 19),
    (po_id, p2, 200, 1200, 19),
    (po_id, p3, 150, 1800, 19),
    (po_id, p4, 300, 900, 19);
  ...
  -- COMPRA 2: aseo, contado (recepción confirmada + CxP que se pagará)
  po_doc := public.next_purchase_number(_company_id,'order');
  ...
  -- Traslado a Punto de Venta: mover algo de stock a wh_pos
  ...
  -- ============ VENTA a crédito (cliente empresa) ============
  ...
  -- ============ POS: turno abierto, 2 ventas, cierre ============
  ...
  -- Cobro parcial de la CxC del cliente 1
